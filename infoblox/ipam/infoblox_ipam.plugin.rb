...
plugin "rs_infoblox_ipam" do
  ...
  endpoint do
    ...
    path "_token/$tunnel_token/wapi/v$api_version/"
  end

  # Infoblox record:host object
  # https://ipam.illinois.edu/wapidoc/objects/record.host.html
  type "record_host" do
    href_templates "/{{contains(_ref, 'record:host') && _ref || null}}" # The leading slash is makes the reference that comes back from infoblox href-like
  ...
  type "a_record" do
    href_templates "/{{contains(_ref, 'record:a') && _ref || null}}"
    provision "provision_record"
    delete "delete_record"
    ...
    action "create" do
      verb "POST"
      path "/record:a?_return_fields=ipv4addr"
    end

    action "search" do
      verb "GET"
      path "/record:a?$search_string"
      type "array"
    ...
    output "_ref","name","ipv4addr"

  end

  type "ptr_record" do
    href_templates "/{{contains(_ref, 'record:ptr') && _ref || null}}"
    provision "provision_record"
    delete "delete_record"
    ...
    action "create" do
      verb "POST"
      path "/record:ptr?_return_fields=ipv4addr"
    end

    output "_ref","name","ipv4addr","ptrdname"
...
define provision_record(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    call sys_log.detail(join(["Name: ", $host_name]))
    @operation = rs_infoblox_ipam.$type.create($fields)
...
define delete_record(@declaration) do
  call start_debugging()
  @declaration.destroy()
  call stop_debugging()
end
