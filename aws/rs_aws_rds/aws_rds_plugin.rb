...
long_description ""
package "plugins/flexera_aws_rds"
import "sys_log"
...
plugin "flexera_aws_rds" do

  short_description 'Amazon Web Services - Relational Database Service'
  long_description 'Supports AWS RDS instances'
  version '2.0.0'

  documentation_link 'source' do
    label 'Source'
    url 'https://github.com/flexera/flexera-plugins/blob/master/aws/rs_aws_rds/aws_rds_plugin.rb'
  end

  documentation_link 'readme' do
    label 'readme'
    url 'https://github.com/flexera/flexera-plugins/blob/master/aws/rs_aws_rds/README.md'
  end
  ...
  type "db_instance" do
    ...
    output 'id' do
     body_path 'DBInstanceIdentifier'
    end
...
define provision_db_instance(@declaration) return @db_instance do
  sub on_error: plugin_generics.stop_debugging() do
    ...
    if $fields["db_snapshot_identifier"] != null 
      @db_instance = flexera_aws_rds.db_instance.create_from_snapshot($fields)
    else
      @db_instance = flexera_aws_rds.db_instance.create($fields)
    end
...
define provision_sg(@declaration) return @sec_group do
  sub on_error: plugin_generics.stop_debugging() do
    ...
    $object = to_object(@declaration)
    $fields = $object["fields"]
    @sec_group = flexera_aws_rds.security_groups.create($fields)
...
define provision_db_subnet_group(@declaration) return @db_subnet_group do
  $object = to_object(@declaration)
  $fields = $object["fields"]
  @db_subnet_group = flexera_aws_rds.db_subnet_groups.create($fields)
